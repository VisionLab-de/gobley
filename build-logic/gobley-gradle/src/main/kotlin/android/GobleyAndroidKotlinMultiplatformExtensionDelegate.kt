...
class GobleyAndroidKotlinMultiplatformExtensionDelegate(
    ...
    // ProGuard/R8 keep-rule injection is not supported for com.android.kotlin.multiplatform.library.
    //
    // Both KmpOptimization.keepRules (DSL) and onVariants (variant API) must be configured
    // during the configuration phase, before afterEvaluate runs. UniFfiPlugin calls
    // addProguardFiles from afterEvaluate, which is too late for both APIs.
    //
    // If keep rules are needed, add them manually in the consumer application's
    // proguardFiles configuration. The rules generated by GenerateUniffiProguardRulesTask
    // (-keep class com.sun.jna.*) can be copied from the build output.
